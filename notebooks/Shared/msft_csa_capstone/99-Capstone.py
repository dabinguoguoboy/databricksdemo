...
def writeToBronze(sourceDataframe, bronzePath, streamName):
    df = sourceDataframe \
    .withColumn("eventDate", to_date(col("eventParams.client_event_time"))).filter(col('eventDate').isNotNull()) 
...
#deviceLookupDF.createOrReplaceTempView("deviceLookupDF")
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
    ...
    df.join(lookupDF, df.device_id == lookupDF.device_id,"inner") \
    .withColumn("deviceType", lookupDF.deviceType) \
    .select(df.device_id,"eventName","client_event_time","eventDate", "deviceType") \
    .writeStream \
    .outputMode("append") \
    .format("delta") \
